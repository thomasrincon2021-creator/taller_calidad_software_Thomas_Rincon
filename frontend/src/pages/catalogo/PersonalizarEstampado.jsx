...
const PersonalizarEstampado = () => {
    ...
    // Si el usuario entra directamente sin seleccionar producto, redirigir al catálogo
    useEffect(() => {
        if (!producto) {
            navigate('/catalogo');
        }
    ...
    // Manejar selección y preview de máximo 3 imágenes
    ...
    const guardarEstampadoEnItem = () => {
        ...
        const datosEstampado = {
            ...
            frases,
            imagenes: imagenes.map(img => img.preview) // En producción enviarías las imágenes subidas al servidor
        };

        // Regresar al catálogo o carrito llevando la info
        navigate('/catalogo', { state: { productoActualizado: { ...producto, estampado: datosEstampado } } });
    };

    const tipoActual = ubicaciones.find(u => u.id === ubicacion)?.tipo || 'simple';

    return (
        <div style={{ backgroundColor: '#09090b', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '1rem' }}>
            <div style={{ backgroundColor: '#18181b', border: '1px solid #27272a', width: '100%', maxWidth: '500px', borderRadius: '1rem', padding: '2rem', boxSizing: 'border-box', position: 'relative', maxHeight: '95vh', overflowY: 'auto' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                    ...
                    {/* Precio Dinámico */}
                    <div style={{ backgroundColor: '#000000', border: '1px solid #27272a', padding: '1rem', borderRadius: '0.5rem', textAlign: 'center', color: 'white', fontWeight: 'bold' }}>
                        Precio Adicional Estampado: <span style={{ color: '#dc2626' }}>${precioExtra.toLocaleString('es-CO')} COP</span>
                    </div>
